...
async function downloadRoutinePDF(){
  // Fetch logo as base64 from the live site
  let logoB64 = null;
  try {
  ...
  if(logoB64){
    ...
    // draw circular clip by drawing a filled white circle then use image on top
    // jsPDF doesn't support path clipping, so draw the image then overlay a ring
    doc.addImage(logoB64, 'JPEG', logoX, logoY, logoSize, logoSize);
    // Silver ring border over logo
