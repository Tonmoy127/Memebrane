...
(function(){
  ...
  function draw(){
    ...
    for(let i = 1; i < n; i++){
      ...
      // Rungs where strands are close to crossing (sin ≈ 0)
      const crossingness = Math.abs(Math.sin(phase1));
      if(crossingness < 0.35 && tMid > 0.08){
        const midX = (s1x1 + s2x1) / 2;
        const midY = (s1y1 + s2y1) / 2;

        ctx.beginPath();
        ctx.moveTo(s1x1, s1y1); ctx.lineTo(s2x1, s2y1);
        ctx.strokeStyle = col.rung + (alpha * 0.6) + ')';
        ctx.globalAlpha = 1;
        ctx.lineWidth = 0.9;
        ctx.stroke();
