...
function renderWritings(){
  ...
  [...list].reverse().forEach(item=>{
    ...
    entry.className = 'writing-entry fade-up visible';
    ...
    entry.innerHTML = `
      <div class="writing-head">
        ...
        <span class="writing-author">${escapeHtml(item.author)}</span>
      </div>
      ${item.image ? `<img class="writing-image" src="${escapeHtml(item.image)}" alt="${escapeHtml(item.title)}">` : ''}
...
function renderPhotography(){
  ...
  [...list].reverse().forEach(item=>{
    ...
    const setSpan = ()=>{
      ...
      // grid-auto-rows is 10px, gap 16px (10px on small screens) - approximate span from rendered column width
      const colWidth = tile.getBoundingClientRect().width || 280;
      const span = Math.max(14, Math.round((colWidth * ratio) / 10));
      tile.style.gridRowEnd = `span ${span}`;
    };
    if(img.complete && img.naturalWidth) setSpan();
    else img.addEventListener('load', setSpan);
    img.addEventListener('error', ()=>{ tile.style.gridRowEnd = 'span 28'; });
  });
}
...
(function(){
  ...
  // and can momentarily report a stale single-axis value. Reading both
  // dimensions from the same layout pass (and debouncing) keeps the
  // canvas buffer's aspect ratio in sync with the viewport, so bubbles
  // drawn as circles via ctx.arc never get stretched into ovals.
  function resize(){
