...
function toggleTheme(){
  ...
  document.getElementById('themeLabel').textContent = isRed ? 'Red' : 'Green';
  document.getElementById('themePip').style.background = isRed ? '#f9c0cc' : '';
...
/* ============================================================
   CUSTOM CURSOR - DNA helix spiral trail
   ============================================================ */
(function(){
  ...
  const TRAIL = 28;       // number of helix segments
  const history = [];     // {x,y} ring buffer
  ...
  function getColors(){
    ...
    return red
      ? { strand1: '#c0334d', strand2: '#f9c0cc', rung: 'rgba(249,192,204,', node: '#f9c0cc' }
      : { strand1: '#6fae7e', strand2: '#e8c873', rung: 'rgba(232,200,115,', node: '#e8c873' };
  }
  ...
  function draw(){
    ...
    if(history.length < 2){ requestAnimationFrame(draw); return; }

    const col = getColors();
    const len = history.length;
    const amplitude = isPointer ? 11 : isDown ? 5 : 8;
    const freq = 0.38; // helix turns per segment

    for(let i = 1; i < len; i++){
      const p0 = history[i-1];
      const p1 = history[i];
      const t  = i / TRAIL;           // 0 (old) → 1 (tip)
      const age = 1 - t;              // fade factor

      // Direction vector along trail
      const dx = p1.x - p0.x;
      const dy = p1.y - p0.y;
      const dist = Math.sqrt(dx*dx + dy*dy) || 1;
      // Perpendicular (for helix oscillation)
      const nx = -dy / dist;
      const ny =  dx / dist;

      // Phase: winds as it goes back
      const phase = (len - i) * freq + tick * 0.04;
      const sinP  = Math.sin(phase)  * amplitude * t;
      const sinP2 = Math.sin(phase + Math.PI) * amplitude * t; // opposite strand

      const s1x = p1.x + nx * sinP;
      const s1y = p1.y + ny * sinP;
      const s2x = p1.x + nx * sinP2;
      const s2y = p1.y + ny * sinP2;

      const alpha = t * 0.85;

      // Draw strand 1 segment
      if(i > 1){
        const pp = history[i-2];
        const pdx = p0.x - pp.x; const pdy = p0.y - pp.y;
        const pd  = Math.sqrt(pdx*pdx+pdy*pdy)||1;
        const pnx = -pdy/pd; const pny = pdx/pd;
        const pPhase = (len - (i-1)) * freq + tick * 0.04;
        const ps1x = p0.x + pnx * Math.sin(pPhase) * amplitude * ((i-1)/TRAIL);
        const ps1y = p0.y + pny * Math.sin(pPhase) * amplitude * ((i-1)/TRAIL);
        const ps2x = p0.x + pnx * Math.sin(pPhase+Math.PI) * amplitude * ((i-1)/TRAIL);
        const ps2y = p0.y + pny * Math.sin(pPhase+Math.PI) * amplitude * ((i-1)/TRAIL);

        ctx.beginPath();
        ctx.moveTo(ps1x, ps1y); ctx.lineTo(s1x, s1y);
        ...
        ctx.beginPath();
        ctx.moveTo(ps2x, ps2y); ctx.lineTo(s2x, s2y);
      ...
      // Rungs - draw where strands cross (every ~3 segments)
      if(i % 3 === 0){
        ctx.beginPath();
        ctx.moveTo(s1x, s1y); ctx.lineTo(s2x, s2y);
        ctx.strokeStyle = col.rung + (alpha * 0.55) + ')';
        ctx.globalAlpha = 1;
        ctx.lineWidth = 1;
        ctx.stroke();
        ...
        ctx.beginPath();
        ctx.arc(s1x, s1y, 1.8 * t, 0, Math.PI*2);
        ctx.fillStyle = col.strand1;
        ctx.globalAlpha = alpha * 0.9;
        ctx.fill();

        ctx.beginPath();
        ctx.arc(s2x, s2y, 1.8 * t, 0, Math.PI*2);
